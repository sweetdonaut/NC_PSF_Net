...
- **無限資料增強**：每次 `__getitem__` 用不同的 defect 位置 / 強度 / 形狀 / case 分配 → 等效樣本量遠大於 250 張底圖
...
1. **底圖密度不可控**：250 張影像中真實前站 defect 數量假設每張 5 個 → 約 1250 個事件。切成 patch 後平均每 patch 只有 0.3 個持續類事件 → **多數 patch 缺乏錨點訓練 signal**
...
3. **資料量**：250 張底圖偏少，真實前站 defect 多樣性是 hybrid 效能的上限
